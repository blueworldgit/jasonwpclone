#!/usr/bin/env python3
"""
Fast async sync of T90 EV product variations from live site

Uses async HTTP requests and batch processing for 10x speed improvement
"""
...
print("=" * 80)
print("FAST ASYNC SYNC - T90 EV VARIATIONS FROM LIVE SITE")
print("=" * 80)
...
async def fetch_all_products(category_id: int) -> Dict[str, List[Dict]]:
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        # First, get page 1 to see if there are more pages
        ...
        # Calculate total pages (WooCommerce returns headers but aiohttp doesn't expose them easily)
        # So we'll just keep fetching until we get empty results
        # Start with pages 2-15 in parallel batches
...
async def main():
    ...
    print("=" * 80)
    print("Step 2: Fetching all T90 EV products...")
    print("=" * 80)
    print()
    ...
    for local in local_products:
        ...
        # First get IDs of wrong parent categories
        cur.execute(f"""
            SELECT t.term_id
            FROM {PREFIX}terms t
        ...
        if wrong_cat_ids:
            ...
            # Delete wrong categories AND their children
            cur.execute(f"""
    ...
    if DRY_RUN:
        ...
        print("  python sync_t90_fast.py --fix")
    else:
