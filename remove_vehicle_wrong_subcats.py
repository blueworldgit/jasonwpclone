#!/usr/bin/env python3
"""
Remove subcategories of wrong parent categories for vehicle products
Usage: python remove_vehicle_wrong_subcats.py <VIN> [--fix]
"""
import mysql.connector
import sys

if len(sys.argv) < 2:
    print("Usage: python remove_vehicle_wrong_subcats.py <VIN> [--fix]")
    print("\nExamples:")
    print("  python remove_vehicle_wrong_subcats.py LSFAM120XNA160733")
    print("  python remove_vehicle_wrong_subcats.py LSH14C4C5NA129710 --fix")
    sys.exit(1)
...
print("=" * 80)
print(f"REMOVE WRONG SUBCATEGORIES FROM {VEHICLE_VIN} PRODUCTS")
print("=" * 80)
...
# Build list of all category IDs to remove (parents + children, excluding electric)
...
for parent_name, info in categories_info.items():
    ...
    if info['subcategories']:
        ...
        print(f"  (no subcategories to remove)")

print(f"\nTotal category IDs to remove: {len(all_wrong_cat_ids)}")
print()

# Find vehicle products that have any of these categories
if all_wrong_cat_ids:
    ...
    """, (VEHICLE_VIN,) + tuple(all_wrong_cat_ids))
    ...
    if len(products) == 0:
        ...
        if DRY_RUN:
            ...
            print(f"  python remove_vehicle_wrong_subcats.py {VEHICLE_VIN} --fix")
