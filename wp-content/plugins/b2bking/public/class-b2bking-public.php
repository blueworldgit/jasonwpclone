...
class B2bking_Public{
			...
			add_action('plugins_loaded', function(){
				...
				if ( defined( 'WC_PLUGIN_FILE' ) && defined('B2BKINGCORE_DIR')) {
					...
					if ( get_option( 'b2bking_plugin_status_setting', 'b2b' ) !== 'disabled' ){
						...
						// validate vat button at checkout blocks
						add_action('wp_footer', [$this, 'validate_vat_button_blocks_checkout']);
						...
						if (!is_user_logged_in()){
							if (get_option('b2bking_guest_access_restriction_setting', 'hide_prices') === 'hide_prices'){	
								...
								add_filter('woocommerce_bundle_empty_price_html', array($this, 'b2bking_hide_prices_guest_users'), 99999, 2 );
								add_action('wp_footer', array($this, 'b2bking_hide_prices_guest_users_bundle_components'));
								...
								if (get_option('b2bking_have_quotes_products_rules', 'yes') === 'yes'){
									...
									if ($this->b2bking_user_is_in_list($user_data_current_user_id, $user_data_current_user_b2b, $user_data_current_user_group, $list) === 'yes'){
										...
										if (apply_filters('b2bking_quote_products_rules_hide_price', true)){
											if (intval(get_option( 'b2bking_hide_prices_quote_only_setting', 1 )) === 1){
												// Hide prices
												add_filter( 'woocommerce_get_price_html', array($this, 'b2bking_hide_prices_request_quote_products'), 99999, 2 );
												add_filter( 'woocommerce_variation_get_price_html', array($this, 'b2bking_hide_prices_request_quote_products'), 99999, 2 );
											}
										}
									...
									add_filter('woocommerce_get_price_html', function($price_html, $product){
										if ($product->is_type('variable')){
											...
											if ($min_price === $max_price){
												...
												$price_html = b2bking()->format_price_range($min_price, $max_price).$product->get_price_suffix();
											}
						...
						if (intval(get_option('b2bking_disable_shipping_control_setting', 0)) === 0){
							...
							// overwrite hide shipping when free to make it work with b2bking
							add_filter('option_woocommerce_shipping_hide_rates_when_free', [$this, 'b2bking_overwrite_hide_shipping_when_free'], 10, 1);
							add_filter('woocommerce_package_rates', [$this, 'b2bking_overwrite_hide_shipping_when_free_higher_priority'], 99999, 2);
						}
					...
					if (get_option( 'b2bking_plugin_status_setting', 'b2b' ) === 'b2b' || (get_option( 'b2bking_plugin_status_setting', 'b2b' ) === 'hybrid' && (get_user_meta( $user_data_current_user_id, 'b2bking_b2buser', true ) === 'yes'))){
						...
						// Credit change price in cart
						if (intval(get_option('b2bking_enable_company_credit_setting', 1)) === 1){
							add_action( 'woocommerce_before_calculate_totals', array($this, 'b2bking_credit_change_price_cart'), 1 );
							add_action( 'woocommerce_before_calculate_totals', array($this, 'b2bking_credit_change_price_cart'), 9999999 );
							add_action( 'woocommerce_after_calculate_totals', array($this, 'b2bking_credit_change_price_cart'), 9999999 );
						}
	...
	function monthly_yearly_group_rules(){
		...
		// New PRO rules engine: short-circuit legacy periodic logic
		if (get_option('b2bking_early_access_group_rules', 'no') === 'yes'){
			if (is_user_logged_in() && class_exists('B2BKing_Group_Rules_Pro')){
				$uid = get_current_user_id();
				$uid = b2bking()->get_top_parent_account($uid);
				$changed = B2BKing_Group_Rules_Pro::apply_group_rules_pro_periodic_for_user($uid);
				if ($changed){
	...
	function b2bking_group_rules_apply($order_id, $posted_data = array(), $order = array()){
		...
		// PRO rules engine enabled?
		$pro_enabled = (get_option('b2bking_early_access_group_rules', 'no') === 'yes' && class_exists('B2BKing_Group_Rules_Pro'));
		if ($pro_enabled){
			$update = (bool) B2BKing_Group_Rules_Pro::apply_group_rules_pro_on_order($order);
		} else {
		...
		// offer purchased by user set (applies to both legacy and PRO flows)
			...
			if ( '' !== $product->get_price() ) {
				...
				if($min_price !== $max_price){
					...
					if (apply_filters('b2bking_tiered_range_show_min', false)){
						$range = '<div class="b2bking_tiered_range_replaced b2bking_tiered_price_range_replaced_'.esc_attr($product->get_id()).'" style="display:none;">'.b2bking()->format_price_range( $min_price, $max_price ).'</div>'.apply_filters('b2bking_tiered_range_show_min_from',esc_html__('from','b2bking')).' '.wc_price($min_price);
					} else {
						$range = '<div class="b2bking_tiered_range_replaced b2bking_tiered_price_range_replaced_'.esc_attr($product->get_id()).'" '.$style.'>'.b2bking()->format_price_range( $min_price, $max_price ).'</div>'.$product->get_price_suffix();
					}
					...
					$range.='<input type="hidden" class="b2bking_tiered_range_original_price_display" value="'.strip_tags(wc_price($max_price)).'">';
				...
				if (!$range){
					...
					if($min_price !== $max_price){						
						...
						if (apply_filters('b2bking_tiered_range_show_min', false)){
							...
							$range = '<div class="b2bking_tiered_range_replaced">'.b2bking()->format_price_range( $min_price, $max_price ).'</div>';
						}
	...
	function b2bking_tiered_raise_integration($price_display, $price, $val, $product_id){
		...
		$disc_amount = b2bking()->get_discount_everywhere_amount($product_id);
		...
		if ($disc_amount > 0){
			if (isset($price_new)){
				// check which one is larger, amount or percentage
				$price_new2 = $price-$disc_amount;
				if ($price_new2 < $price_new){
					$price_new = $price_new2;
				}
			} else {
				$price_new = $price-$disc_amount;
			}
		}

		if ($disc_amount > 0 || $disc_percentage > 0){
	...
	public static function b2bking_show_custom_information_table($prodidshortcode = ""){
		...
		if ($product){
			...
			if ($b2bking_has_run2 === false || $shortcodeusage === true || apply_filters('b2bking_may_show_info_table_multiple', false)){

				if ((is_object($product) && intval($post_id) === intval(b2bking()->safe_get_queried_object_id())) || $shortcodeusage === true || apply_filters('b2bking_may_show_info_table_multiple', false)) {
		...
		if (!is_object( WC()->cart )){
	    	...
	    	foreach( WC()->cart->get_cart() as $cart_item_key => $cart_item ){
				...
				if ($product->get_id() === $product_id || $product->get_id() === $variation_id){
					...
					if (isset($cart_item['acfw_add_product_discount_type']) && $cart_item['acfw_add_product_discount_type'] === 'override'){
						if(isset($cart_item['acfw_add_product_discount_value']) && floatval($cart_item['acfw_add_product_discount_value']) === floatval(0)){
							return $price;
						}
					}

					if (apply_filters('b2bking_custom_cart_item_ignore_condition', false, $cart_item)){
						return $price;
					}
	...
	function b2bking_custom_woocommerce_billing_fields_blocks(){
		...
		if ( ! apply_filters('b2bking_blocks_billing_data_apply_force', false)){ // can cause performance issues on large sites
		...
		// Check if custom mapping filter is enabled for cache key
		...
		// Try to get cached custom fields
		$custom_fields = get_transient($cache_key);
		
		if (false === $custom_fields) {
			// Cache miss - get all enabled custom fields with no default billing connection (first name, last name etc)
			$custom_fields = get_posts([
	    		'post_type' => 'b2bking_custom_field',
	    	  	'post_status' => 'publish',
	    	  	'numberposts' => -1,
		...
		foreach ($custom_fields as $custom_field){
			...
			if ($field_type !== 'file'){ // not available to files for the moment
				...
				if ($field_type === 'number' || $field_type === 'email' || $field_type === 'tel'){
					$field_type = 'text';
				}
			    ...
			    if ($field_type === 'select'){
			    	...
			    	foreach ($choices_array as $choice){
			    		...
			    		if (count($optionvalue) === 2 ){
			    			// value is specified
			    			$options_array[trim($optionvalue[0])] = array('label' => trim($optionvalue[0]), 'value' => trim($optionvalue[1]));
			    		} else {
			    			// simple
			    			$options_array[trim($choice)] = array('label' => trim($choice), 'value' => trim($choice));
			    		}
			    	}
			    }
			    ...
			    if (!empty($field_array['options'])){
			    	$blocks_args['options'] = $field_array['options'];
			    }
	...
	// add custom fields to order meta
	public static function b2bking_add_custom_fields_to_order_meta( $order_id, $rest_request = false ) {
		...
		// get and save meta data from billing postmeta
		if (is_object($order)){ // wc order object
			$order_meta = $order->get_meta_data();

			// Loop through each meta data item
			foreach ($order_meta as $meta_item) {
			    // Get the key from the meta data object
			    $meta_key = $meta_item->get_data()['key'];
			    $meta_value = $meta_item->get_data()['value'];
			    
			    // Check if the key contains 'b2bking'
			    if (strpos($meta_key, 'b2bking') !== false) {
			        // Extract the part after the last slash
			        $key_parts = explode('/', $meta_key);
			        $simplified_key = end($key_parts);
			        
			        // Save as new order meta with the simplified key
			        $order->update_meta_data($simplified_key, $meta_value);
			    }
			}

			// Save the order to persist the meta data changes
			$order->save();
		}
				...
				foreach ($custom_fields as $custom_field){
					...
					if ($billing_exclusive !== 1){
						...
						if ($billing_connection !== 'billing_countrystate' && $billing_connection !== 'billing_country' && $billing_connection !== 'billing_vat'){
							...
							// if there's only one country, force state options
							if (get_option('woocommerce_allowed_countries') === 'specific'){
								$countries = get_option('woocommerce_specific_allowed_countries');
								if (is_array($countries)){
									if (count($countries) === 1){
										$_POST[ 'billing_country' ] = $countries[0];
									}
								}
							}
	...
	function b2bking_custom_registration_fields_check_errors( $errors, $username, $email ) {
		...
		if (isset($_POST['b2bking_registration_roles_dropdown'])){
    	    ...
    	    foreach ($custom_fields as $field_id){
    	    	...
    	    	if (empty($value) && isset($_POST['b2bking_custom_field_'.$field_id])){
	...
	function b2bking_display_custom_registration_fields(){
		...
		foreach ($custom_fields as $field){
			...
			if ($field_type !== 'file' && $field_type !== 'checkbox' && $field_type !== 'radio'){
				...
				// display label
				echo '<label for="b2bking_custom_field_'.esc_attr($field->ID).'">'.esc_html($field_label).'</label>';

				// display field
				if ($field_type === 'text'){
					echo '<input type="text" class="b2bking_custom_registration_field" name="b2bking_custom_field_'.esc_attr($field->ID).'" id="b2bking_custom_field_'.esc_attr($field->ID).'" value="'.esc_attr($field_value).'" '.$customval.'><br /><br />';
				} else if ($field_type === 'number'){
					echo '<input type="number" step="0.00001" class="b2bking_custom_registration_field" name="b2bking_custom_field_'.esc_attr($field->ID).'" id="b2bking_custom_field_'.esc_attr($field->ID).'" value="'.esc_attr($field_value).'"><br /><br />';
				} else if ($field_type === 'email'){
					echo '<input type="email" class="b2bking_custom_registration_field" name="b2bking_custom_field_'.esc_attr($field->ID).'" id="b2bking_custom_field_'.esc_attr($field->ID).'" value="'.esc_attr($field_value).'"><br /><br />';
				} else if ($field_type === 'date'){
					echo '<input type="date" class="b2bking_custom_registration_field" name="b2bking_custom_field_'.esc_attr($field->ID).'" id="b2bking_custom_field_'.esc_attr($field->ID).'" value="'.esc_attr($field_value).'"><br /><br />';
				} else if ($field_type === 'time'){
					echo '<input type="time" class="b2bking_custom_registration_field" name="b2bking_custom_field_'.esc_attr($field->ID).'" id="b2bking_custom_field_'.esc_attr($field->ID).'" value="'.esc_attr($field_value).'"><br /><br />';
				} else if ($field_type === 'tel'){
					echo '<input type="tel" class="b2bking_custom_registration_field" name="b2bking_custom_field_'.esc_attr($field->ID).'" id="b2bking_custom_field_'.esc_attr($field->ID).'" value="'.esc_attr($field_value).'"><br /><br />';
				} else if ($field_type === 'textarea'){
					echo '<textarea class="b2bking_custom_registration_field" name="b2bking_custom_field_'.esc_attr($field->ID).'" id="b2bking_custom_field_'.esc_attr($field->ID).'">'.esc_html($field_value).'</textarea><br /><br />';
				} else if ($field_type === 'select'){
					$user_options = explode(',', $field_user_choices);
					echo '<select class="b2bking_custom_registration_field" name="b2bking_custom_field_'.esc_attr($field->ID).'" id="b2bking_custom_field_'.esc_attr($field->ID).'">';
	...
	function b2bking_overwrite_hide_shipping_when_free($val){

		if ($val === 'yes'){
			// overwrite with B2BKing function
			global $b2bking_check_package_rates;

			// not b2bking checking the setting
			if ($b2bking_check_package_rates !== 'yes'){
				$val = 'no';
			}
		}
	...
	function b2bking_overwrite_hide_shipping_when_free_higher_priority($rates, $package){
		global $b2bking_check_package_rates;
		$b2bking_check_package_rates = 'yes';
		$hide_shipping_when_free = get_option('woocommerce_shipping_hide_rates_when_free','no');

		if ($hide_shipping_when_free === 'yes'){
			// overwrite without custom function that does the same thing just with higher priority
		...
		$b2bking_check_package_rates = 'no';
		return $rates;
	}
	    		...
	    		foreach ($available as $rate_id => $rate){
	    			...
	    			foreach ($smpr_rules as $rule){
	    				...
	    				if ($rate_id === $rule_shippingmethod){
    						...
    						if ($rule_applies === 'cart_total'){
							...
							} else if ($rule_applies === 'excluding_multiple_options'){
								// check that any product in cart is not in exclusion list
								$multiple_options = get_post_meta($rule, 'b2bking_rule_applies_multiple_options', true);
								$multiple_options_array = explode(',', $multiple_options);

								$has_non_excluded_product = false;
								...
								if (is_object( WC()->cart )){
									foreach(WC()->cart->get_cart() as $cart_item){
										...
										// check if product has any variations that are excluded
										// If current product is a variation, check parent's children; otherwise check current product's children
										$product_to_check_children = ($variation_parent_id !== 0) ? $variation_parent_id : $current_product_id;
										$productobjj = wc_get_product($product_to_check_children);
										if ($productobjj){
										...
										if ($product_is_excluded === 'no'){
											...
											$has_non_excluded_product = true;
											break;
										}
									}
								}

								if ($has_non_excluded_product){
									unset($available[$rate_id]);
								}
							} else {
	...
	function validate_vat_button_blocks_checkout() {
		...
		if (intval(get_option('b2bking_validate_vat_button_checkout_setting', 0)) === 1){
			...
			if ($b2bking_has_shown_validate_vat !== 'yes'){
					...
					document.addEventListener('DOMContentLoaded', function() {
				        ...
				        function addContentAfterBillingCheckbox() {
				            ...
				            if (billingCheckbox && !document.getElementById('custom-message-after-billing')) {
				                ...
				                customDiv.innerHTML = '<button type="button" id="b2bking_checkout_registration_validate_vat_button" <?php echo $disabled; ?>><?php echo esc_html($textvat); ?></button>';
				...
				if (!empty($custom_fields)){
					echo '<input type="hidden" id="b2bking_vat_number_registration_field_number" name="b2bking_vat_number_registration_field_number" value="'.esc_attr($custom_fields[0]->ID).'">';
				}
			...
			if (!empty($pmd_rules)){
				...
				if ($maximum !== 'no'){
					if (is_object( WC()->cart )){
						...
						foreach ($payment_methods as $payment_method){
							if ($payment_method->id === $gateway_id){
								if (isset($payment_method->title) && !empty($payment_method->title)){
									...
									$method_title = $payment_method->get_title();
								}
							}
						}
						$method_title = strip_tags($method_title);
						...
						// add surcharge / discount suffix if it doesn't already exist
						if (strpos($method_title, $text_after) === false) {
							$text_final = apply_filters('b2bking_text_payment_method_discount_final', $method_title.' '.$text_after);
						} else {
							$text_final = apply_filters('b2bking_text_payment_method_discount_final', $method_title);
						}
	...
	function b2bking_credit_change_price_cart( $_cart ){
		// loop through the cart_contents
	    foreach ( $_cart->cart_contents as $cart_item_key => $value ) {
	...
	function b2bking_hide_offer_post($query) {
		...
		if (is_array($current_exclude)){
			$query->query_vars['post__not_in'] = array_merge(array($offer_id, $credit_id, $mkcredit_id), $current_exclude); 
		} else {
        	$query->query_vars['post__not_in'] = array($offer_id, $credit_id, $mkcredit_id);
    	}
	}	
			...
			<div id="b2bking_conversation_messages_container">
			    ...
			    <div class="b2bking_conversation_message <?php echo esc_attr($self); ?>">
			    	...
			    	<div class="b2bking_conversation_message_time">
			    		...
			    		<?php echo esc_html(apply_filters('b2bking_display_message_author', $author)).' - '; ?>
				...
				if (b2bking()->has_b2b_application_pending(get_current_user_id())){
					...
					if (!b2bking()->is_b2b_user()){
						...
						<form action="<?php echo esc_url( admin_url('admin-post.php') ); ?>" method="POST" class="woocommerce-form woocommerce-form-register register">
							...
							<input type="hidden" name="redirectto" value="<?php echo add_query_arg('b2b_applied', '1', home_url($_SERVER['REQUEST_URI'])); ?>">
						...
						echo '<span class="b2bking_already_logged_in_message">';
						if (isset($_GET['b2b_applied']) && $_GET['b2b_applied'] === '1'){
							esc_html_e('Thank you for your submission. Your B2B account has been approved. You may now browse our products and place orders. ', 'b2bking');
							echo '<a href="' . esc_url(wc_get_page_permalink('shop')) . '">' . esc_html__('Visit Shop', 'b2bking') . '</a>';
						} else {
							esc_html_e('Your user account is already a B2B account.','b2bking');
						}
						echo '</span>';
	...
	function b2bking_handle_file_upload(){
		...
		if ( $error !== 'no') {
			...
			$csv = array_map(function($v) {
			    return str_getcsv($v, ',', '"', '\\');
			}, file($_FILES['b2bking_csvorder']['tmp_name'], FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES));
	...
	function myaccount_query_vars ($vars) {
		...
		$endpoints = [get_option('b2bking_conversations_endpoint_setting','conversation'), get_option('b2bking_conversations_endpoint_setting','conversations'), get_option('b2bking_offers_endpoint_setting','offers'), get_option('b2bking_bulkorder_endpoint_setting','bulkorder'), get_option('b2bking_subaccount_endpoint_setting','subaccount'), get_option('b2bking_subaccounts_endpoint_setting','subaccounts'), get_option('b2bking_purchaselist_endpoint_setting','purchase-list'), get_option('b2bking_purchaselists_endpoint_setting','purchase-lists')];

		if (intval(get_option('b2bking_enable_company_credit_setting', 1)) === 1 && defined('b2bkingcredit_DIR')){
			$endpoints[] = get_option('b2bking_credit_endpoint_setting', 'company-credit');
		}
	...
	*/
	...
	function b2bking_offers_shortcode_content($atts){
	    ...
	    $shortcode_offer_id = sanitize_text_field($atts['id']);
		...
		if (is_user_logged_in()){
		    ...
		    if ( $custom_query->have_posts() ) {
		        while ( $custom_query->have_posts() ) {
		            ...
		            if (apply_filters('b2bking_offer_skip', false, $post->ID, get_current_user_id())){
		            	continue;
		            }
		            ...
		            <div class="b2bking_myaccount_individual_offer_container">
		            	...
		            	foreach ($offer_products as $product){
		            		...
		            		<div class="b2bking_myaccount_individual_offer_element_line">
		            			...
		            			?>"><div class="b2bking_myaccount_individual_offer_element_line_item_name"><?php echo esc_html(strip_tags($product_details[0])); ?>
		            			<?php
		            			// Check if this product needs configuration and add dropdowns inline
		            			if ($isproductid[0] === 'product'){
		            					...
		            					foreach ($productobj->get_variation_attributes() as $name => $value){
		            						if (empty($value) && $value !== 0 && $value !== '0') {
		            							wc_dropdown_variation_attribute_options(array(
		            								'attribute' => str_replace('attribute_', '', $name),
		            								'product'   => wc_get_product($productobj->get_parent_id()),
		            								'class'		=> 'variation_'.$isproductid[1].' b2bking_offer_select',
		            								'required'	=> true,
		            								'show_option_none' => esc_html__('Choose ','b2bking').wc_attribute_label(str_replace('attribute_', '', $name)),
		            								'id'		=> str_replace('attribute_', '', $name).'_'.$isproductid[1],
		            								'name'		=> $name.'_'.$isproductid[1]
		            							));
		            						}
		            					}
		            				}
	...
	function b2bking_variations_table_shortcode_content( $atts ){
	    ...
	    if (!empty($product_id)){
	    	...
	    	$sanitized_hidecolumns = sanitize_text_field($hidecolumns);
	    	$sanitized_width = sanitize_text_field($width);
	    	echo do_shortcode(apply_filters('b2bking_bulk_variations_table_shortcode', '[b2bking_bulkorder theme=cream variationstable=yes product_list="'.$sanitized_product_id.'" multiselect="yes" style="'.$sanitized_style.'" hidecolumns="'.$sanitized_hidecolumns.'" width="'.$sanitized_width.'"]'));
	    	?>
	    	<style type="text/css">
	...
	function b2bking_bulkorder_shortcode_content( $atts ){
	    ...
	    $multiselect = sanitize_text_field($atts['multiselect']);
	    ...
	    $is_variations_table = sanitize_text_field($atts['variationstable']);
	    $attributes = sanitize_text_field($atts['attributes']);
	    $width = sanitize_text_field($atts['width']);
	    $hidecolumns = sanitize_text_field($atts['hidecolumns']);
	    ...
	    if (empty($hidecolumns)){
	    	...
	    	$hidecolumns = array_map('sanitize_text_field', $hidecolumns);
	    }

	    $attributestext = sanitize_text_field($atts['attributestext']);
	    $showsku = sanitize_text_field($atts['sku']);
	    $showstock = sanitize_text_field($atts['stock']);
	    ...
	    $product_list = sanitize_text_field($atts['product_list']);
	    $sortby = sanitize_text_field($atts['sortby']);
	    $instock = sanitize_text_field($atts['instock']);
		...
		?>
		<div class="b2bking_bulkorder_container_final <?php if ($is_variations_table === 'yes'){ echo 'variations_table'; } ?>">
			...
			if ($theme === 'cream'){
				...
				<div class="b2bking_bulkorder_form_container <?php echo 'multiselect_'.esc_attr($multiselect).' '; ?><?php echo esc_attr($width); ?> b2bking_bulkorder_form_container_indigo b2bking_bulkorder_form_container_cream b2bking_orderform_<?php echo get_option( 'b2bking_order_form_creme_cart_button_setting', 'cart' );?>_container <?php 
					...
					<div class="b2bking_bulkorder_cream_header_container">
						...
						// prepopulate based on query args
						$search = '';
						if (apply_filters('b2bking_prepopulate_bulkorder_search', true)){
							if (isset($_GET['search'])){
								$search = sanitize_text_field($_GET['search']);
							} else if (isset($_GET['b2bkingsearch'])){
								$search = sanitize_text_field($_GET['b2bkingsearch']);
							}
						}
						...
						<div class="b2bking_bulkorder_form_container_content_header_top b2bking_bulkorder_form_container_content_header_top_cream">
							<input type="text" id="b2bking_bulkorder_search_text_indigoid" class="b2bking_bulkorder_search_text_indigo b2bking_bulkorder_search_text_cream" placeholder="<?php esc_html_e('Search products...','b2bking');?>" value="<?php echo $search; ?>">
							...
							<div class="b2bking_bulkorder_form_container_cream_filters_content_first">
								...
								if (apply_filters('b2bking_bulkorder_instock_filter', false)){
									...
									<ul class="b2bking_bulkorder_filters_list_instock">
										...
										<?php if (apply_filters('b2bking_bulkorder_instock_filter_show_onbackorder', false)): ?>
										<li value="onbackorder"><?php esc_html_e('On Backorder', 'b2bking'); ?></li>
										<?php endif; ?>
	...
	function b2bking_subaccounts_endpoint_content(){
		...
		<div class="b2bking_subaccounts_container">
				...
				if (apply_filters('b2bking_allow_subaccount_creation_editing', true)){
					...
					<div class="b2bking_subaccounts_new_account_container">
						...
						<div class="b2bking_subaccounts_new_account_container_content">
							...
							if (apply_filters('b2bking_disable_username_subaccounts', 1) === 0 || apply_filters('b2bking_show_username_subaccounts', false)){
								?>
							...
							<div class="b2bking_subaccounts_new_account_container_content_element b2bking_subaccount_horizontal_line" <?php
							...
							?>>
								...
								<input type="password" class="b2bking_subaccounts_new_account_container_content_element_text" name="b2bking_subaccounts_new_account_password" placeholder="<?php esc_attr_e('Enter the subaccount password here...','b2bking'); ?>" <?php 

								if (apply_filters('b2bking_subaccounts_set_own_password', false)){
									echo ' value="account123456" <!-- not actually used, subaccount will set own password -->';
								}
							?>>
							</div>
			...
			foreach($subaccounts_array as $subaccount){
				...
				// verify that the user indeed belongs to current parent
				$parent_of_subaccount = get_user_meta($subaccount, 'b2bking_account_parent', true);
				if (intval($parent_of_subaccount) !== intval($user_id)){
					$user_subaccounts_list = str_replace(','.$subaccount,'',$user_subaccounts_list);
					update_user_meta($user_id, 'b2bking_subaccounts_list', sanitize_text_field($user_subaccounts_list));
					continue;
				}
    ...
    function b2bking_bulk_variations_table_replace_after(){
    	...
    	if ($is_enabled === 'yes' && $location === 'replace'){
    		...
    		?>
    		<style type="text/css">
    			.woocommerce-variation-add-to-cart.variations_button > *:not(.b2bking_information_table){
    				display: none !important;
    			}
    		...
    		<script>
    			jQuery(document).ready(function(){
    				jQuery('.b2bking_information_table').detach().insertAfter('.b2bking_bulkorder_form_cream_main_container');
    			});
    		</script>
    		<?php
    		if ($attributes_nr === 1){
		...
		if (in_array ( $subaccount_id, $current_user_subaccounts)){
			...
			<div class="b2bking_subaccounts_edit_account_container">
				...
				<div class="b2bking_subaccounts_new_account_container_content">
					...
					do_action('b2bking_custom_subaccount_after_permissions', $subaccount_id);
		...
		if (in_array($list_author_id, $subaccounts_list )){
				...
				<div class="b2bking_bulkorder_form_container_content">
					...
					<?php 
						...
						foreach ($list_items as $list_item){
							...
							// Check if product is valid - if not, mark for cleanup and skip
							if (!is_a($productobj,'WC_Product_Variation') && !is_a($productobj,'WC_Product')){
								$cleanup_needed = true;
								continue; // Skip this deleted product
							}

							// Add valid product to cleaned list
							$valid_list_items[] = $list_item;
						...
						// Clean up deleted products from the list if needed
						if ($cleanup_needed && !empty($valid_list_items)) {
							$cleaned_list_details = implode('|', $valid_list_items);
							update_post_meta($purchase_list_id, 'b2bking_purchase_list_details', $cleaned_list_details);
						}
			...
			if ($purchase_lists_theme !== 'classic'){
				add_filter('b2bking_purchase_list_display', function($list, $purchase_list_id, $products_list_comma, $list_title){
					...
					$shortcode = apply_filters('b2bking_purchase_list_shortcode','[b2bking_bulkorder width="nonadaptive" multiselect="'.$multiselect.'" theme='.$purchase_lists_theme.' product_list="'.$products_list_comma.'"]');
	...
	function b2bking_hide_prices_guest_users_bundle_components(){
		?>
		<style>
	...
	function b2bking_product_redirection_to_account() {
		...
		if (!is_user_logged_in() || (intval(get_option( 'b2bking_multisite_separate_b2bb2c_setting', 0 )) === 1 && get_user_meta(get_current_user_id(), 'b2bking_b2buser', true) !== 'yes')){	
		    ...
		    // Get the redirect URL and check if it's still the my account page
		    $redirect_url = apply_filters('b2bking_forbidden_access_redirection_url', get_permalink( wc_get_page_id( 'myaccount' ) ));
		    $myaccount_url = get_permalink( wc_get_page_id( 'myaccount' ) );
		    
		    // Prevent infinite loop if already on my account page and redirect URL is still my account
		    if ( is_account_page() && $redirect_url === $myaccount_url ) return;
		...
		if (is_page() || is_shop() || is_home()){
			if ( get_option( 'b2bking_plugin_status_setting', 'b2b' ) !== 'disabled' ){
				...
				if ( ! $has_access ){
					...
					// if current user is SK agent AND page to be accessed is agent dashboard page, auto-enabe
					if (apply_filters('b2bking_agent_can_always_view_dashboard_page', true)){
						if ( is_user_logged_in() ) {
						    ...
						    if ($is_sales_agent === 'none' || empty($is_sales_agent)){
						    	...
						    	// check if current page is agent dashboard page
						    	global $post;
						    	if (isset($post->ID)){
			    ...
			    if ( ! $has_access ){
			    	...
			    	// Prevent infinite loop if already on my account page and redirect URL is still my account
			    	if ( ! ( is_account_page() && $redirect_url === $myaccount_url ) ){
					   wp_redirect( 
					       add_query_arg( 
					           'redirect_to', 
					           urlencode( $_SERVER['REQUEST_URI'] ), 
	...
	function b2bking_categories_restrict( $args, $taxonomies ) {
		...
		if (apply_filters('b2bking_categories_restrict_filter_abort', false)){
	        return $args;
	    }
			...
			if (!get_transient('b2bking_display_price_both'.$product_id.'_'.get_current_user_id()) || apply_filters('b2bking_display_both_cache_disable', true)){
				...
				if ($is_b2b_user === 'yes'){
					...
					if( $product->is_type('variable') ){
						...
						if ($min_price !== $max_price){
							$retail_price = b2bking()->format_price_range( $min_price, $max_price );
						} else {
					...
					if (apply_filters('b2bking_show_both_prices_all', false)){
						...
						if( $product->is_type('variable') ){
							...
							// if min and max prices are different, show range, else show the price
							if ($min_price !== $max_price){
								$wholesale_price = b2bking()->format_price_range( $min_price, $max_price );
							} else {
	...
	function b2bking_add_request_quote_button($location = 'standard', $product_id = '', $button_or_form = 'button'){
		...
		if (intval(get_option('b2bking_enable_conversations_setting', 1)) === 1){
			if ((get_option('b2bking_guest_access_restriction_setting', 'hide_prices') === 'replace_prices_quote') && (!is_user_logged_in() || (intval(get_option( 'b2bking_multisite_separate_b2bb2c_setting', 0 )) === 1 && get_user_meta(get_current_user_id(), 'b2bking_b2buser', true) !== 'yes')) ){
				...
				// must also auto-enable button if there's a quote on specific products only item in cart
				if (b2bking()->user_has_p_in_cart('quote') === 'yes'){
					$quote_button_setting = 'enableall';
	...
	function enqueue_public_resources(){
	    ...
	    */
		...
		if (get_option( 'b2bking_plugin_status_setting', 'b2b' ) === 'b2b' || (get_option( 'b2bking_plugin_status_setting', 'b2b' ) === 'hybrid' && (get_user_meta( $user_data_current_user_id, 'b2bking_b2buser', true ) === 'yes'))){
			...
			if (is_user_logged_in()){
				if (apply_filters('b2bking_load_datatables', true)){
    	...
    	$data_to_be_passed = array(
    		...
    		'credit_text_translat1' => esc_html__('Available credit: ','b2bking'),
    		'credit_text_translat2' => esc_html__('Insufficient credit - your available credit is: ','b2bking'),
    		'credit_text_translat3' => esc_html__('You can ','b2bking'),
    		'credit_text_translat4' => esc_html__('redeem credits','b2bking'),
    		'credit_text_translat5' => esc_html__(' as a coupon.','b2bking'),
		);
