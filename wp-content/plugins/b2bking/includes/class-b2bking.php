...
class B2bking {
	...
	function handle_form_become_b2b_loggedin() {
	    ...
	    if (isset($_POST['redirectto'])){
	        ...
	        if ($redirect_parsed && isset($redirect_parsed['host']) && 
	...
	public function b2bkingemailoffer(){
		...
		// Check if PDF attachment is requested via checkbox
		$attach_pdf = isset($_POST['attach_pdf']) && $_POST['attach_pdf'] === '1';
		$temp_pdf_path = null;
		
		// Process PDF if provided and attachment is requested
		if ($attach_pdf && isset($_POST['pdfdata']) && !empty($_POST['pdfdata'])) {
			$pdf_base64 = wp_unslash($_POST['pdfdata']);
			$pdf_base64 = preg_replace('/\s+/', '', $pdf_base64);
			$pdf_binary = base64_decode($pdf_base64, true);
			
			if ($pdf_binary !== false && strlen($pdf_binary) > 0) {
				$upload_dir = wp_upload_dir();
				$temp_dir = $upload_dir['basedir'] . '/b2bking-temp/';
				
				if (!file_exists($temp_dir)) {
					wp_mkdir_p($temp_dir);
				}
				
				$filename = 'offer-' . $offer_id . '-' . time() . '.pdf';
				$temp_pdf_path = $temp_dir . $filename;
				
				file_put_contents($temp_pdf_path, $pdf_binary);
			}
		}
		...
		// Store PDF path in transient (in case action params don't work)
		if ($temp_pdf_path) {
			set_transient('b2bking_offer_pdf_' . $offer_id, $temp_pdf_path, 300);
		}
		
		foreach ($emails_send_to as $emailad){
			do_action( 'b2bking_new_offer', $emailad, '1', $offer_id, $offer_link, $temp_pdf_path );
		}
		foreach ($emails_send_to_guest as $emailad){
			do_action( 'b2bking_new_offer', $emailad, '0', $offer_id, $offer_link, $temp_pdf_path );
		}
	...
	function b2bking_approve_order(){
		...
		if (apply_filters('b2bking_approve_order_only_direct_parent', true)){		
            	...
            	foreach ($product_ids as $product_id){
            		...
            		if ($productobj !== false){
	            		...
	            		<div class="b2bking_bulkorder_form_container_content_line b2bking_bulkorder_form_container_content_line_indigo">
	            			...
	            			do_action('b2bking_bulkorder_column_header_mid_content', $productobj); ?><div class="b2bking_bulkorder_form_container_content_line_subtotal b2bking_bulkorder_form_container_content_line_subtotal_indigo"><?php 
			            ...
			            if ($possible_parent_id !== 0){
			            	...
			            	if (!in_array($parent_id, $displayed_parents)){
			            		...
			            		<div class="b2bking_bulkorder_form_container_content_line b2bking_bulkorder_form_container_content_line_indigo b2bking_bulkorder_form_container_content_line_cream b2bking_bulkorder_form_container_content_line_cream_view_options">
			            			...
			            			?><div class="b2bking_bulkorder_form_container_content_line_subtotal b2bking_bulkorder_form_container_content_line_subtotal_indigo b2bking_bulkorder_form_container_content_line_subtotal_cream"><?php 
			            			...
			            			if ($logged_out_hide_prices){
			            				...
			            				if ($this->dynamic_replace_prices_with_quotes() === 'yes' || (get_option('b2bking_guest_access_restriction_setting', 'hide_prices') === 'replace_prices_quote') && (!is_user_logged_in() || (intval(get_option( 'b2bking_multisite_separate_b2bb2c_setting', 0 )) === 1 && get_user_meta($currentuserid, 'b2bking_b2buser', true) !== 'yes'))){
			            					...
			            					if (b2bking()->product_has_quote_rule($parentobj->get_id())){
			            						...
			            						if ((defined('WOOCS_VERSION') || apply_filters('b2bking_bulk_cream_recalculate_from_price', false)) && b2bking()->is_b2b_user()) {
			            							$min_price = -1;
	...
	function b2bkingbulksetsubaccounts(){
		...
		foreach ($subaccount_ids as $subaccount_id){
			...
			// Check if subaccount already has a parent, and remove from old parent's list if different
			$old_parent_id = get_user_meta($subaccount_id_trimmed, 'b2bking_account_parent', true);
			if (!empty($old_parent_id) && $old_parent_id != $parent_id) {
				$old_parent_subaccounts_list = get_user_meta($old_parent_id, 'b2bking_subaccounts_list', true);
				if (!empty($old_parent_subaccounts_list)) {
					// Remove subaccount from old parent's list using array operations for exact matching
					$old_parent_subaccounts_array = array_filter(explode(',', $old_parent_subaccounts_list));
					$old_parent_subaccounts_array = array_diff($old_parent_subaccounts_array, array($subaccount_id_trimmed));
					update_user_meta($old_parent_id, 'b2bking_subaccounts_list', implode(',', $old_parent_subaccounts_array));
				}
			}
			...
			// Add to new parent's list using array operations for exact matching
			$current_subaccounts_list = get_user_meta($parent_id,'b2bking_subaccounts_list', true);
			$current_subaccounts_array = !empty($current_subaccounts_list) ? array_filter(explode(',', $current_subaccounts_list)) : array();
			if (!in_array($subaccount_id_trimmed, $current_subaccounts_array)) {
				$current_subaccounts_array[] = $subaccount_id_trimmed;
				update_user_meta($parent_id,'b2bking_subaccounts_list', implode(',', $current_subaccounts_array));
			}
	...
	function filter_get_stock_status_callback( $stock_status, $product ){
	   ...
	   // return is_admin() ? $stock_status : apply_filters('b2bking_always_in_stock_status', 'instock'); // can also be onbackorder
		return apply_filters('b2bking_always_in_stock_status', 'instock', $stock_status);	
	}
