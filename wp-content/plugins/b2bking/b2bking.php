...
if (strlen($host_names[count($host_names)-2]) <= 3 && isset($host_names[count($host_names)-3])) {
	$bottom_host_name = $host_names[count($host_names)-3] . "." . $host_names[count($host_names)-2] . "." . $host_names[count($host_names)-1];
}
update_option('pluginactivation_'.$email.'_'.$license.'_'.$bottom_host_name, 'active');
update_option('b2bking_license_email_setting', $email);
update_option('b2bking_license_key_setting', $license);
update_option('b2bking_use_legacy_activation', 'no');
update_option('b2bking_failed_license_'.$license, 0);
add_filter('pre_http_request', function($pre, $parsed_args, $url) {
	if (strpos($url, 'kingsplugins.com/wp-json/licensing') !== false) {
		$email = 'noreply@gmail.com';
		$license = 'B5E0B5F8DD8689E6ACA49DD6E6E1A930';
		$info = parse_url(get_site_url());
		$host = $info['host'];
		$host_names = explode(".", $host);
		$bottom_host_name = $host_names[count($host_names)-2] . "." . $host_names[count($host_names)-1];
		if (strlen($host_names[count($host_names)-2]) <= 3 && isset($host_names[count($host_names)-3])) { $bottom_host_name = $host_names[count($host_names)-3] . "." . $host_names[count($host_names)-2] . "." . $host_names[count($host_names)-1]; } update_option('pluginactivation_'.$email.'_'.$license.'_'.$bottom_host_name, 'active'); update_option('b2bking_license_email_setting', $email); update_option('b2bking_license_key_setting', $license); update_option('b2bking_use_legacy_activation', 'no'); update_option('b2bking_failed_license_'.$license, 0); return ['body' => 'success', 'response' => ['code' => 200, 'message' => 'OK']];
	}
	return $pre;
}, 10, 3);

if ( ! defined( 'B2BKING_VERSION' ) ) {
	define(	'B2BKING_VERSION', 'v5.4.70');
}
