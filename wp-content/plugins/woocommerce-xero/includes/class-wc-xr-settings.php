...
class WC_XR_Settings {
	...
	public function __construct( $override = null ) {
		...
		$this->defaults = array(
			...
			'payment_account'          => '',
			'rounding_account'         => '',
			'send_invoices'            => 'manual',
			'send_payments'            => 'off',
			'treat_shipping_as'        => 'expense',
			'treat_fees_as'            => 'expense',
			'branding_theme'           => '',
			'match_zero_vat_tax_rates' => 'off',
			'four_decimals'            => 'off',
			'export_zero_amount'       => 'off',
			'send_inventory'           => 'off',
			'append_email_to_contact'  => 'on',
			'debug'                    => 'off',
		);
	 ...
	 * @return string $capability Capability being checked.
	 */
	public function add_save_capability( $capability ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
	...
	public function get_option( $key ) {
		...
		return get_option( self::OPTION_PREFIX . $key, $this->defaults[ $key ] ?? '' );
	}
	...
	public function init_settings() {
		...
		$this->settings = array(
			...
			'client_id'                => array(
				...
				'default'     => $this->defaults['client_id'] ?? '',
				'type'        => 'text_oauth',
			...
			'client_secret'            => array(
				...
				'default'     => $this->defaults['client_secret'] ?? '',
				'type'        => 'password',
				'description' => __( 'OAuth Credential retrieved from <a href="https://developer.xero.com/myapps/" target="_blank">Xero Developer My Apps Centre</a>.', 'woocommerce-xero' ),
			),
			...
			'oauth_20'                 => array(
				...
				'default'     => $this->defaults['oauth_20'] ?? '',
				'type'        => 'oauth',
			...
			'invoice_prefix'           => array(
				...
				'default'     => $this->defaults['invoice_prefix'] ?? '',
				'type'        => 'text',
			...
			'sales_account'            => array(
				...
				'default'     => $this->defaults['sales_account'] ?? '',
				'type'        => 'text',
			...
			'shipping_account'         => array(
				...
				'default'     => $this->defaults['shipping_account'] ?? '',
				'type'        => 'text',
			...
			'fees_account'             => array(
				...
				'default'     => $this->defaults['fees_account'] ?? '',
				'type'        => 'text',
			...
			'payment_account'          => array(
				...
				'default'     => $this->defaults['payment_account'] ?? '',
				'type'        => 'text',
			...
			'rounding_account'         => array(
				...
				'default'     => $this->defaults['rounding_account'] ?? '',
				'type'        => 'text',
			...
			'send_invoices'            => array(
				...
				'default'     => $this->defaults['send_invoices'] ?? 'manual',
				'type'        => 'select',
			...
			'send_payments'            => array(
				...
				'default'     => $this->defaults['send_payments'] ?? 'off',
				'type'        => 'select',
			...
			'treat_shipping_as'        => array(
				...
				'default'     => $this->defaults['treat_shipping_as'] ?? 'expense',
				'type'        => 'select',
			...
			'treat_fees_as'            => array(
				...
				'default'     => $this->defaults['treat_fees_as'] ?? 'expense',
				'type'        => 'select',
			...
			'branding_theme'           => array(
				...
				'default'     => $this->defaults['branding_theme'] ?? '',
				'type'        => 'select',
			...
			'match_zero_vat_tax_rates' => array(
				...
				'default'     => $this->defaults['match_zero_vat_tax_rates'] ?? 'off',
				'type'        => 'checkbox',
			...
			'four_decimals'            => array(
				...
				'default'     => $this->defaults['four_decimals'] ?? 'off',
				'type'        => 'checkbox',
			...
			'export_zero_amount'       => array(
				...
				'default'     => $this->defaults['export_zero_amount'] ?? 'off',
				'type'        => 'checkbox',
			...
			'send_inventory'           => array(
				...
				'default'     => $this->defaults['send_inventory'] ?? 'off',
				'type'        => 'checkbox',
			...
			'append_email_to_contact'  => array(
				...
				'default'     => $this->defaults['append_email_to_contact'] ?? 'on',
				'type'        => 'checkbox',
			...
			'debug'                    => array(
				...
				'default'     => $this->defaults['debug'] ?? 'off',
				'type'        => 'checkbox',
				'description' => sprintf(
	...
	public function add_menu_item() {
		...
		add_action( 'load-' . $sub_menu_page, array( $this, 'enqueue_xero_settings_script' ) );
	}
	...
	/**
	 * Enqueue scripts used by Xero settings page.
	 */
	public function enqueue_xero_settings_script() {
		wp_enqueue_script(
			'woocommerce_xero_settings',
			plugin_dir_url( WC_Xero::get_plugin_file() ) . 'assets/js/admin-settings.js',
			array( 'jquery' ),
			WC_XERO_VERSION,
			true
		);
		...
		wp_localize_script(
			'woocommerce_xero_settings',
			'woocommerce_xero_settings_params',
			array(
				'i18n_nav_warning' => __( 'The changes you made will be lost if you navigate away from this page.', 'woocommerce-xero' ),
			)
		);
	}
		...
		if ( ! isset( $_GET['code'] ) ) {
			...
			try {
				...
				if ( 'invalid_grant' === $e->getResponseBody()['error'] ) {
					...
					$log_message = 'Error while connecting to Xero account: ' . $e->getMessage() . "\n" . $e->getTraceAsString();
					$wc_logger->add( 'xero', $log_message, WC_Log_Levels::ERROR );
					?>
					<div class="wrap woocommerce">
					<div class="icon32 icon32-woocommerce-settings" id="icon-woocommerce"><br/></div>
							<h2><?php esc_html_e( 'Xero OAuth', 'woocommerce-xero' ); ?></h2>
							<p><strong><?php esc_html_e( 'Callback failed:', 'woocommerce-xero' ); ?></strong></p>
							<p><?php esc_html_e( 'An unexpected error occurred. Please try again later and review the logs for detailed error information.', 'woocommerce-xero' ); ?></p>
					</div>
					<?php
				}
			}
