...
use Automattic\WooCommerce\Xero\Vendor\GuzzleHttp\Exception\ConnectException;
use Automattic\WooCommerce\Xero\Vendor\GuzzleHttp\Exception\RequestException;
use Automattic\WooCommerce\Xero\Vendor\GuzzleHttp\Promise as P;
use Automattic\WooCommerce\Xero\Vendor\GuzzleHttp\Promise\FulfilledPromise;
use Automattic\WooCommerce\Xero\Vendor\GuzzleHttp\Promise\PromiseInterface;
use Automattic\WooCommerce\Xero\Vendor\GuzzleHttp\Psr7\LazyOpenStream;
use Automattic\WooCommerce\Xero\Vendor\GuzzleHttp\TransferStats;
use Automattic\WooCommerce\Xero\Vendor\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \Automattic\WooCommerce\Xero\Vendor\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \Automattic\WooCommerce\Xero\Vendor\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \Automattic\WooCommerce\Xero\Vendor\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \Automattic\WooCommerce\Xero\Vendor\GuzzleHttp\Psr7\Utils::streamFor($sink);
